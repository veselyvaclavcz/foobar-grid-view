// Album Art Grid v9.9.9 FINAL - Complete with Shutdown Protection
...
// Global shutdown flag for safety (defined in initquit_v999.cpp)
extern std::atomic<bool> g_component_shutting_down;
...
DECLARE_COMPONENT_VERSION(
    "Album Art Grid",
    "9.9.9",
    "Album Art Grid v9.9.9 - Shutdown Crash Fix\n"
    "CRITICAL FIX in v9.9.9:\n"
    "- Fixed crashes during shutdown/sleep\n"
    "- Fixed access violations on exit\n"
    "- Improved callback unregistration\n"
    "- Protected window message handling\n"
    "\n"
    "Previous features from v9.9.8:\n"
    "NEW in v9.9.6:\n"
...
struct grid_item {
    ...
    pfc::string8 get_display_text(grid_config::label_format format) const {
        if (format == grid_config::LABEL_ARTIST_ALBUM && !artist.is_empty() && !album.is_empty()) {
            pfc::string8 result;
            result << artist << " - " << album;
            return result;
        }
...
private:
    std::atomic<bool> m_is_destroying;  // Shutdown protection flag
    HWND m_hwnd;
    HWND m_parent;
...
public:
    album_grid_instance(ui_element_config::ptr config, ui_element_instance_callback_ptr callback)
        : m_callback(callback), m_hwnd(NULL), m_parent(NULL), m_tooltip(NULL), m_search_box(NULL),
          ...
          m_last_user_scroll(0), m_is_destroying(false) {
        m_config.load(config);
        // Don't register callbacks in constructor - wait for window creation
    }
    ...
    LRESULT on_contextmenu(int x, int y) {
        ...
        AppendMenu(label_menu, MF_STRING | (m_config.label_style == grid_config::LABEL_ALBUM_ONLY ? MF_CHECKED : 0), 37, TEXT("Album Only"));
        AppendMenu(label_menu, MF_STRING | (m_config.label_style == grid_config::LABEL_ARTIST_ALBUM ? MF_CHECKED : 0), 38, TEXT("Artist - Album"));
        AppendMenu(menu, MF_POPUP, (UINT_PTR)label_menu, TEXT("Label Format"));
