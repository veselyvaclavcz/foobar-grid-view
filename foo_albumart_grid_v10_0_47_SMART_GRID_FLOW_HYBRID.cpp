// Album Art Grid v10.0.47 - SMART GRID FLOW (stability + async loads)
...
DECLARE_COMPONENT_VERSION(
    "Album Art Grid",
    "10.0.47",
    "Album Art Grid v10.0.47 - SMART GRID FLOW\n"
...
struct grid_config {
    ...
    ui_element_config::ptr save(const GUID& guid) {
        ...
        append(auto_scroll_to_now_playing); append(enlarged_now_playing); append(show_playlist_overlay);
    ...
    void load(ui_element_config::ptr cfg) {
        ...
        if (sz >= (need/2)) {
            ...
            auto_scroll_to_now_playing = tmp.auto_scroll; enlarged_now_playing = (enlarged_mode)std::clamp(tmp.enlarged, 0, 2); show_playlist_overlay = tmp.overlay;
        }
    }
};
...
public:
    ...
    void draw_item(HDC hdc, Gdiplus::Graphics& graphics, HFONT font,
        ...
        if (item->thumbnail->bitmap) {
            // Prefer low-cost sampling when no scaling is required
            if (item->thumbnail->cached_size == item_size) {
