# PowerShell build script for v10.0.18 MPV COMPATIBLE - foo_mpv Plugin Compatibility Fix
Write-Host "===========================================" -ForegroundColor Green
Write-Host "Building v10.0.18 MPV COMPATIBLE - foo_mpv Compatibility Fix" -ForegroundColor Green
Write-Host "CRITICAL: Fixed mpv plugin 'libav error for open input: Invalid argument'" -ForegroundColor Green
Write-Host "===========================================" -ForegroundColor Green
Write-Host ""
...
Write-Host "Compiling v10.0.18 MPV COMPATIBLE with simple callback delays..." -ForegroundColor Yellow
Write-Host "CRITICAL FIXES:" -ForegroundColor Yellow
Write-Host "- Fixed mpv plugin race condition causing 'libav error for open input: Invalid argument'" -ForegroundColor Yellow
Write-Host "- Simple 200ms delay in playlist callbacks prevents interference with video playback" -ForegroundColor Yellow
Write-Host "- Maintains all v10.0.18 shutdown fixes" -ForegroundColor Yellow
Write-Host "- Zero performance impact when not playing video" -ForegroundColor Yellow
Write-Host ""
...
$compileArgs = @(
    ...
    "/I$SDK_PATH\pfc",
    "/Fo:build\",
    "temp_fixed_base_v10_0_18_MPV_COMPATIBLE_FIXED.cpp"
)
...
$linkArgs = @(
    "/DLL",
    "/OUT:build\foo_albumart_grid_v10_0_18_MPV_COMPATIBLE.dll",
    "/MACHINE:X64",
...
if (Test-Path "build\foo_albumart_grid_v10_0_18_MPV_COMPATIBLE.dll") {
    Write-Host ""
    Write-Host "===========================================" -ForegroundColor Green
    Write-Host "BUILD SUCCESSFUL!" -ForegroundColor Green
    Write-Host "===========================================" -ForegroundColor Green
    
    Get-ChildItem "build\foo_albumart_grid_v10_0_18_MPV_COMPATIBLE.dll"
    
    Copy-Item "build\foo_albumart_grid_v10_0_18_MPV_COMPATIBLE.dll" "foo_albumart_grid.dll"
    
    Compress-Archive -Path "foo_albumart_grid.dll" -DestinationPath "foo_albumart_grid_v10_0_18_MPV_COMPATIBLE.zip" -Force
    Move-Item "foo_albumart_grid_v10_0_18_MPV_COMPATIBLE.zip" "foo_albumart_grid_v10_0_18_MPV_COMPATIBLE.fb2k-component" -Force
    
    Write-Host ""
    Write-Host "MPV COMPATIBILITY FIXED COMPONENT READY!" -ForegroundColor Green -BackgroundColor DarkGreen
    Write-Host "foo_albumart_grid_v10_0_18_MPV_COMPATIBLE.fb2k-component" -ForegroundColor Green
    Get-ChildItem "foo_albumart_grid_v10_0_18_MPV_COMPATIBLE.fb2k-component"
    Write-Host ""
