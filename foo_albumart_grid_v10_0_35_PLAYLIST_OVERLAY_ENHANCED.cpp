// Album Art Grid v10.0.30 - PERFORMANCE OPTIMIZATION
// NEW: Critical performance fixes for large collections (3500+ items)
// - Layout caching eliminates expensive recalculations during scrolling
// - O(1) mathematical hit testing replaces O(n) iteration
// - Context menu text lazy loading (eliminates 1-2 second delay)
// - Scroll throttling with 60 FPS for smooth operation
// MAINTAINED: All v10.0.29 enlarged now playing features
...
DECLARE_COMPONENT_VERSION(
    "Album Art Grid",
    "10.0.35",
    "Album Art Grid v10.0.35 - PLAYLIST OVERLAY ENHANCED\n"
    "NEW in v10.0.35 ENHANCED:\n"
    "- NEW: Playlist overlay for 3x3 enlarged now playing artwork\n"
    "- NEW: Shows ACTUAL PLAYLIST tracks (not album tracks)\n"
    "- NEW: Complete track info: Number, Artist, Title, Duration, Codec\n"
    "- NEW: Current track highlighting with blue color and play indicator\n"
    "- NEW: Smart scrolling when track list is longer than display area\n"
    "- NEW: Configurable via 'Show Playlist Overlay' in Enlarged Now Playing menu\n"
    "MAINTAINED from v10.0.34:\n"
    "- Perfect theme color logic for text overlays\n"
    "- Text overlay directly on artwork - no layout problems with enlarged artwork\n"
    "- Grid layout preserved - enlarged artwork remains square\n"
    "MAINTAINED from v10.0.32:\n"
    "- Page Up/Down navigation using PgUp/PgDn keys\n"
    "- Letter Jump navigation (A-Z, 0-9) working\n"
    "- Full Unicode support with proper UTF-8 to UTF-16 conversion\n"
    "MAINTAINED from v10.0.30 - PERFORMANCE OPTIMIZATION:\n"
    "- Layout caching eliminates expensive recalculations during scrolling\n"
    "- O(1) mathematical hit testing replaces O(n) iteration\n"
    "- Scroll throttling with 60 FPS for smooth operation\n"
    "- Massive speed improvement for scrollbar and right-click response\n"
    "MAINTAINED from v10.0.29 - ENLARGED NOW PLAYING:\n"
    "NEW FEATURE in v10.0.29:\n"
    "- Enlarged now playing artwork (configurable 2x2/3x3 size)\n"
    "- Adaptive grid layout - other items reposition around enlarged artwork\n"
    "- Enhanced menu with 'Enlarged Now Playing' submenu\n"
    "- Smart hit testing for variable-sized items\n"
    "MAINTAINED from v10.0.28:\n"
    "- Destructor shutdown detection and protection\n"
    "- Immortal callback handlers for crash prevention\n"
    "From v10.0.26 DEDICATED PLAYLIST:\n"
    "- Single reusable 'Album Grid' playlist\n"
    "- Direct menu access via right-click\n"
    "- Streamlined workflow\n"
    "CRITICAL FIX in v10.0.20:\n"
    "- FIXED: Proper service_impl_t inheritance for correct reference counting\n"
    "- FIXED: Component stays alive until foo_ui_std releases it\n"
    "- FIXED: No more vtable crashes during shutdown\n"
    "From v10.0.19:\n"
    "- VTable protection with validated_object\n"
    "From v10.0.18:\n"
    "- FIXED: NULL pointer write crash during shutdown (0x0000000000000000)\n"
    "- FIXED: Proper destructor sequence - invalidate BEFORE DestroyWindow\n"
    "- FIXED: Window pointer cleared before destruction\n"
    "- FIXED: Callbacks unregistered before window destruction\n"
    "- FIXED: Window procedure checks instance validity\n"
    "From v10.0.17:\n"
    "- Object validation with magic numbers\n"
    "- Protected pointer access with SEH\n"
    "From v10.0.16:\n"
    "- FIXED: Unicode characters now display correctly in all labels (Chinese, Japanese, etc.)\n"
    "- FIXED: main_thread_callback::callback_run crash during shutdown\n" 
    "- Enhanced callback validity checks with exception handling\n"
    "- Improved text rendering with proper Unicode support\n"
    "From v10.0.15:\n"
    ...
    "- Fixed version string reporting (was incorrectly showing v10.0.17)\n"
...
namespace shutdown_protection {
    static volatile bool g_is_shutting_down = false;
...
T safe_virtual_call(Func&& func, T default_val) {
    ...
    __except(EXCEPTION_EXECUTE_HANDLER) {
        console::printf("[Album Art Grid v10.0.17] Exception in safe_virtual_call setup");
        return default_val;
    }
...
struct grid_config {
    ...
    bool auto_scroll_to_now_playing;  // NEW v10.0.27: Auto-scroll to now playing
    
    // NEW v10.0.29: Enlarged now playing feature
    enum enlarged_mode {
        ENLARGED_NONE = 0,    // No enlargement (default)
        ENLARGED_2X2 = 1,     // 2x2 enlarged artwork  
        ENLARGED_3X3 = 2      // 3x3 enlarged artwork
    };
    enlarged_mode enlarged_now_playing;

    // NEW v10.0.35: Playlist overlay for enlarged now playing artwork
    bool show_playlist_overlay;
    ...
    grid_config() : columns(5), text_lines(2), show_text(true), show_track_count(true),
                    ...
                    doubleclick(DOUBLECLICK_PLAY), label_style(LABEL_ALBUM_ONLY), auto_scroll_to_now_playing(false),
                    enlarged_now_playing(ENLARGED_NONE), show_playlist_overlay(false) {}
    ...
    void load(ui_element_config::ptr cfg) {
        if (cfg.is_valid()) {
            ...
            if (actual_size >= expected_size - sizeof(view_mode) - sizeof(doubleclick_action)) {
                memcpy(this, cfg->get_data(), std::min(actual_size, expected_size));
                ...
                columns = std::max(1, columns); // No upper limit - unlimited columns!
                text_lines = std::max(1, std::min(3, text_lines));
                font_size = std::max(7, std::min(14, font_size));
...
struct thumbnail_data {
    ...
    void clear() {
        if (bitmap) {
            // v10.0.28 FIX: Check if GDI+ is still available during shutdown
            HWND main_wnd = core_api::get_main_window();
            if (!main_wnd || !IsWindow(main_wnd)) {
                // During shutdown - GDI+ might be unavailable, just nullify pointer
                console::print("[Album Art Grid v10.0.28] SHUTDOWN DETECTED - skipping GDI+ bitmap cleanup");
                bitmap = nullptr;
                memory_size = 0;
                return;
            }
...
private:
    ...
    static volatile bool shutdown_in_progress;  // v10.0.9: F9FCh crash fix
    ...
    static size_t get_available_memory() {
        ...
        return std::max(min_size, std::min(max_size, quarter_available));
    }
...
volatile bool thumbnail_cache::shutdown_in_progress = false;  // v10.0.9: F9FCh fix
...
// Grid UI element instance - v10.0.20: BACK TO service_impl_t WITH PROPER CLEANUP
// CRITICAL FIX: Remove service_impl_t to prevent vtable corruption
...
private:
    ...
    volatile bool m_is_destroying;  // Atomic flag to prevent use-after-free in playlist callbacks
    bool m_tracking;
    ...
    struct context_menu_cache {
        ...
        bool is_valid_for(int index) const {
            return valid && cached_index == index &&
                   (GetTickCount() - last_update) < 5000; // 5 second cache
        }
    ...
    DWORD m_last_scroll_update;
    static const DWORD SCROLL_THROTTLE_MS = 16; // 60 FPS
...
public:
    ...
    // v10.0.30: NEW - Simple and reliable adaptive layout for enlarged items
    item_position get_adaptive_item_position(int index, const RECT& client_rect, int start_x, int cols, int text_height) {
        item_position pos = {0, 0, 0, 0, false};

        int enlarged_index = m_now_playing_index;
        if (enlarged_index < 0 || enlarged_index >= (int)m_items.size()) {
        ...
        int multiplier = (m_config.enlarged_now_playing == grid_config::ENLARGED_2X2) ? 2 : 3;
        int item_height = m_item_size + text_height + PADDING;

        // SIMPLE APPROACH: Create a virtual layout map and place items sequentially
        // Calculate how many items we have
        size_t total_items = get_item_count();

        // Create layout map - estimate maximum rows needed
        int max_rows = ((int)total_items + cols - 1) / cols + multiplier;
        std::vector<std::vector<bool>> occupied(max_rows, std::vector<bool>(cols, false));

        // Calculate where enlarged item should be placed
        // Start from its natural position but adjust if it doesn't fit
        int enlarged_row = enlarged_index / cols;
        int enlarged_col = enlarged_index % cols;

        // Make sure enlarged item fits in grid - if not, move it to a position where it fits
        if (enlarged_col + multiplier > cols) {
            enlarged_col = cols - multiplier;
            if (enlarged_col < 0) enlarged_col = 0;
        }

        // Mark occupied cells for enlarged item
        for (int r = enlarged_row; r < enlarged_row + multiplier && r < max_rows; r++) {
            for (int c = enlarged_col; c < enlarged_col + multiplier && c < cols; c++) {
                occupied[r][c] = true;
            }
        }

        // Now find position for current item
        if (index == enlarged_index) {
            // This is the enlarged item
            pos.x = start_x + enlarged_col * (m_item_size + PADDING) + PADDING;
            pos.y = enlarged_row * item_height + PADDING - m_scroll_pos;
            pos.width = multiplier * m_item_size + (multiplier - 1) * PADDING;
            pos.height = multiplier * m_item_size + text_height;
            pos.visible = (pos.y + pos.height > 0 && pos.y < client_rect.bottom);
            return pos;
        }

        // For other items, place them sequentially in first available slot
        // We need to account for the fact that the enlarged item "takes up" its original position
        int items_to_place = 0; // How many regular items we need to place before reaching our target

        for (int i = 0; i < (int)total_items; i++) {
            if (i == enlarged_index) continue; // Skip the enlarged item
            if (i == index) break; // Found our target item
            items_to_place++;
        }
        ...
        for (int r = 0; r < max_rows; r++) {
            for (int c = 0; c < cols; c++) {
                if (!occupied[r][c]) {
                    if (placed_items == items_to_place) {
                        // This is our item's position
                        pos.x = start_x + c * (m_item_size + PADDING) + PADDING;
                        pos.y = r * item_height + PADDING - m_scroll_pos;
                        pos.width = get_item_size(index);
                        pos.height = get_item_size(index) + text_height;
                        pos.visible = (pos.y + pos.height > 0 && pos.y < client_rect.bottom);
                        return pos;
        ...
        pos.x = start_x + PADDING;
        pos.y = PADDING - m_scroll_pos;
        pos.width = get_item_size(index);
        pos.height = get_item_size(index) + text_height;
        pos.visible = (pos.y + pos.height > 0 && pos.y < client_rect.bottom);
        return pos;
    ...
    Gdiplus::Bitmap* create_original_artwork(metadb_handle_ptr track, int target_size) {
        if (!track.is_valid() || m_is_destroying || !m_hwnd) {
            return nullptr;
        }
            ...
            try {
                ...
                try {
                    ...
                    if (item->artwork.is_valid()) {
                        ...
                        // v10.0.30: Use high-res artwork for enlarged items, regular thumbnails for others
                        int current_item_size = get_item_size(i);
                        Gdiplus::Bitmap* bmp = nullptr;
                        try {
                            if (i == m_now_playing_index && m_config.enlarged_now_playing != grid_config::ENLARGED_NONE) {
                                // For enlarged now playing - load original artwork directly from track
                                bmp = create_original_artwork(item->tracks[0], current_item_size * 2);  // Even higher target size
                            } else {
                                // For normal items - use regular thumbnail
                                bmp = create_thumbnail(item->artwork, current_item_size);
                            }
                        } catch(...) {
                            bmp = nullptr;
                        }
                        ...
                        if (bmp && !shutdown_protection::g_is_shutting_down) {
                            {
                                ...
                                if (item && item->thumbnail) {
                                    item->thumbnail->set_bitmap(bmp, current_item_size);
                                }
                            }
                            if (item && item->thumbnail) {
                            ...
                            console::print("[Album Art Grid v10.0.16] Failed to create thumbnail");
                        }
                    }
                } catch (...) {}
            } catch (...) {}
                ...
                try {
                    ...
                    try {
                        ...
                        if (item->artwork.is_valid()) {
                            ...
                            // v10.0.30: Use high-res for enlarged items in preload too
                            if (i == m_now_playing_index && m_config.enlarged_now_playing != grid_config::ENLARGED_NONE) {
                                bmp = create_original_artwork(item->tracks[0], current_item_size * 2);
                            } else {
                                bmp = create_thumbnail(item->artwork, current_item_size);
                            }
    ...
    LRESULT on_paint() {
        ...
        // v10.0.30: Draw floating label near mouse cursor when hovering over an item
        // This provides enhanced label that follows the mouse cursor
        if (m_hover_index >= 0 && m_hover_index < (int)get_item_count()) {
            draw_floating_label(memdc, hFont, m_hover_index, m_mouse_x, m_mouse_y, rc);
        }
    ...
    void draw_playlist_overlay(HDC hdc, Gdiplus::Graphics& graphics, HFONT font,
        ...
        if (!m_config.show_playlist_overlay) return;
        ...
        int overlay_height = item_size - 40; // Leave margin for text overlay at bottom
        ...
        int line_height = tm.tmHeight + tm.tmExternalLeading + 2;

        // Calculate how many tracks can fit
        int max_tracks = (overlay_height - 20) / line_height;  // Leave margins
        int track_count = (int)playlist_count;
        int start_track = 0;

        // If there are more tracks than fit, center around current playing track
        if (has_current_track && track_count > max_tracks) {
            ...
            for (int i = 0; i < track_count; i++) {
                ...
                if (playlist_manager->playlist_get_item_handle(playlist_track, active_playlist, i)) {
                    if (playlist_track == current_track) {
                        ...
                        start_track = std::min(start_track, track_count - max_tracks);
                        break;
                    }
                }
            }
        ...
        // Header text
        TextOutA(hdc, col1_x, header_y, "#", 1);
        TextOutA(hdc, col2_x, header_y, "Title", 5);
        TextOutA(hdc, col3_x, header_y, "Artist", 6);
        TextOutA(hdc, col4_x, header_y, "Time", 4);

        // Header separator line
        HPEN line_pen = CreatePen(PS_SOLID, 1, RGB(100, 100, 100));
        HPEN old_pen = (HPEN)SelectObject(hdc, line_pen);
        int line_y = header_y + line_height - 2;
        ...
        // Draw tracks starting below header
        int content_start_y = header_y + line_height + 5;
        for (int i = 0; i < std::min(max_tracks - 1, track_count); i++) { // -1 for header
            ...
            // Draw current track indicator (play triangle)
            if (is_current) {
                SetTextColor(hdc, RGB(100, 200, 255));
                TextOutA(hdc, overlay_x + 5, row_y, ">", 1);
                SetTextColor(hdc, text_color);
            }
            ...
            TextOutA(hdc, col1_x, row_y, padded_num.c_str(), padded_num.get_length());

            // Draw title in column 2 (with ellipsis if too long)
            RECT title_rect = {col2_x, row_y, col3_x - 10, row_y + line_height};
            DrawTextA(hdc, title_str.c_str(), -1, &title_rect,
                     DT_LEFT | DT_VCENTER | DT_SINGLELINE | DT_END_ELLIPSIS);

            // Draw artist in column 3 (with ellipsis if too long)
            RECT artist_rect = {col3_x, row_y, col4_x - 10, row_y + line_height};
            DrawTextA(hdc, artist_str.c_str(), -1, &artist_rect,
                     DT_LEFT | DT_VCENTER | DT_SINGLELINE | DT_END_ELLIPSIS);

            // Draw duration in column 4
            if (length > 0) {
                int minutes = (int)(length / 60);
                int seconds = (int)(length) % 60;
                pfc::string8 time_str;
                time_str << pfc::format_int(minutes) << ":" << pfc::format_int(seconds, 2);
                TextOutA(hdc, col4_x, row_y, time_str.c_str(), time_str.get_length());
            }

        }

        // Draw footer with total time and track count
        int footer_y = content_start_y + (std::min(max_tracks - 1, track_count)) * line_height + 10;
        ...
        if (total_time > 0) {
            ...
            TextOutA(hdc, overlay_x + 15, footer_y + 5, footer_text.c_str(), footer_text.get_length());
        }
        ...
        if (track_count > max_tracks) {
            ...
            indicator_text << "(" << (start_track + 1) << "-" << (start_track + std::min(max_tracks, track_count))
                          << " of " << track_count << ")";
    ...
    void draw_item(HDC hdc, Gdiplus::Graphics& graphics, HFONT font,
        ...
        if (item->thumbnail->bitmap) {
            // NEW v10.0.29: Scale image to item_size (enlarged if now playing)
            ...
            HFONT icon_font = CreateFont(item_size / 4, 0, 0, 0, FW_NORMAL, FALSE, FALSE, FALSE,
                DEFAULT_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS,
        ...
        if (is_now_playing) {
            ...
            int icon_size = std::min(24, item_size / 8);  // Smaller size, similar to badge
            int icon_x = x + 5;  // Bottom-left position
            int icon_y = y + item_size - icon_size - 5;
        ...
        // v10.0.33: Text overlay implementation - draw text directly on artwork
        // with theme-appropriate semi-transparent background for readability
        if (m_config.show_text) {
            ...
            // Convert UTF-8 to wide string for proper Unicode display
            std::wstring wide_text;
            int wide_len = MultiByteToWideChar(CP_UTF8, 0, text_to_show.c_str(), -1, NULL, 0);
            if (wide_len > 0) {
                wide_text.resize(wide_len - 1);  // -1 to exclude null terminator
                MultiByteToWideChar(CP_UTF8, 0, text_to_show.c_str(), -1, &wide_text[0], wide_len);
            }
    ...
    LRESULT on_vscroll(int code) {
        // v10.0.30: PERFORMANCE OPTIMIZATION - Scroll throttling for smooth 60 FPS
        DWORD current_time = GetTickCount();
        if (current_time - m_last_scroll_update < SCROLL_THROTTLE_MS) {
            // Skip this scroll event to maintain 60 FPS
            return 0;
        }
        ...
        calculate_layout();  // Update m_item_size (now cached!)
        int text_height = calculate_text_height();
        int item_total_height = m_item_size + text_height;
        int cols = m_config.columns;
        size_t item_count = get_item_count();
        ...
        // v10.0.30: Add extra height ONLY for enlarged items in the last row (same as update_scrollbar)
        if (m_config.enlarged_now_playing != grid_config::ENLARGED_NONE &&
            m_now_playing_index >= 0 && m_now_playing_index < (int)item_count) {

            int now_playing_row = m_now_playing_index / std::max(1, cols);

            // Only add extra height if the enlarged item is in the very last row
            if (now_playing_row == rows - 1) {
                int enlarged_size = get_item_size(m_now_playing_index);
                int extra_height = enlarged_size - m_item_size;

                if (extra_height > 0) {
                    total_height += extra_height; // Just the extra height needed
                }
            }
        }
        ...
        m_last_user_scroll = GetTickCount();
        return 0;
    }
    ...
    void update_scrollbar() {
        ...
        // v10.0.30: Add extra height ONLY for enlarged items in the last row
        if (m_config.enlarged_now_playing != grid_config::ENLARGED_NONE &&
            m_now_playing_index >= 0 && m_now_playing_index < (int)item_count) {
    ...
    void check_now_playing() {
        ...
        try {
            ...
            // Store last track for change detection
            static metadb_handle_ptr last_track;

            if (pc->get_now_playing(track)) {
                // Only update if the track actually changed
                if (!track.is_valid() || !last_track.is_valid() ||
                    track != last_track) {
                    console::printf("Track changed - updating now playing");
                    update_now_playing(track);
                    last_track = track;
                }
            } else if (last_track.is_valid()) {
                // Playback stopped
                console::printf("Playback stopped");
...
private:
    ...
    void jump_to_now_playing(bool animate = true) {
        ...
        // Calculate target scroll position considering enlarged artwork
        int row = m_now_playing_index / std::max(1, m_config.columns);
        int text_height = calculate_text_height();
        int normal_item_height = m_item_size + text_height;
        int target_scroll = row * (normal_item_height + PADDING);
...
volatile bool initquit_gdiplus::s_shutting_down = false;
...
private:
    ...
    static volatile bool s_being_destroyed;
...
volatile bool album_art_grid_library_viewer::s_being_destroyed = false;
