// Album Art Grid v10.0.43 - SMART GRID FLOW
...
DECLARE_COMPONENT_VERSION(
    ...
    "Album Art Grid v10.0.43 - SMART GRID FLOW\n"
    "NEW in v10.0.43:\n"
    "- SMART FLOW: Intelligent enlarged item placement with backward positioning\n"
    "- AUTO REFLOW: Automatic grid reflow to prevent gaps at end\n"
    "- BLOCKAGE CALC: Smart blockage calculation for seamless item flow\n"
    "- PERFECT GRID: Perfect grid filling even with last item enlarged\n"
...
public:
    ...
    void rebuild_placement_map(int cols) const {
        ...
        // v10.0.43 SMART GRID FLOW: Calculate exact total grid size
        int total_cells_needed = (int)total_items;
        if (enlarged_index >= 0 && enlarged_index < (int)total_items) {
            // Add extra cells for enlarged item (3x3 needs 8 extra, 2x2 needs 3 extra)
            total_cells_needed += (multiplier * multiplier - 1);
        }

        // Calculate exact grid dimensions - no more rows than necessary
        int total_rows = (total_cells_needed + cols - 1) / cols; // Ceiling division
        int cells_in_last_row = total_cells_needed % cols;
        if (cells_in_last_row == 0) cells_in_last_row = cols; // Full last row

        m_cell_map.resize(total_rows, std::vector<GridCell>(cols));

        // Find "last usable position" for enlarged item
        int enlarged_row = -1, enlarged_col = -1;
        int last_usable_position = -1;

        if (enlarged_index >= 0 && enlarged_index < (int)total_items) {
            // Find the last position where 3x3 can fit without using forbidden zones
            find_last_usable_position(total_rows, cols, multiplier, cells_in_last_row, last_usable_position, enlarged_row, enlarged_col);
        }

        // Place items with "wrap-around" logic for items that would come after enlarged item
        int grid_position = 0;
        int wrapped_items_placed = 0;

        // First pass: Place items that come after enlarged item in "wrapped" positions
        if (enlarged_index >= 0 && last_usable_position >= 0) {
            for (int i = enlarged_index + 1; i < (int)total_items; i++) {
                // These items wrap to front of grid
                while (grid_position < last_usable_position && is_position_blocked(grid_position, total_rows, cols, enlarged_row, enlarged_col, multiplier)) {
                    grid_position++;
                }

                if (grid_position < last_usable_position) {
                    int row = grid_position / cols;
                    int col = grid_position % cols;
                    m_item_placements[i] = ItemPlacement(row, col, 1, 1);
                    m_cell_map[row][col].occupied = true;
                    m_cell_map[row][col].item_index = i;
                    m_cell_map[row][col].is_primary = true;
                    grid_position++;
                    wrapped_items_placed++;
                }
            }
        }

        // Reset position for main sequence
        grid_position = wrapped_items_placed;

        // Second pass: Place remaining items in sequence
        for (int i = 0; i < (int)total_items; i++) {
            if (i == enlarged_index) {
                // Store enlarged item placement at last usable position
                m_item_placements[i] = ItemPlacement(enlarged_row, enlarged_col, multiplier, multiplier);
                // Mark cells as occupied
                for (int r = enlarged_row; r < enlarged_row + multiplier; r++) {
                    for (int c = enlarged_col; c < enlarged_col + multiplier; c++) {
                        m_cell_map[r][c].occupied = true;
                        m_cell_map[r][c].item_index = i;
                        m_cell_map[r][c].is_primary = (r == enlarged_row && c == enlarged_col);
                    }
                }
            } else if (i > enlarged_index) {
                // These were already placed in first pass (wrapped)
                continue;
            } else {
                // Place normal items before enlarged item
                while (grid_position < total_rows * cols && is_position_blocked(grid_position, total_rows, cols, enlarged_row, enlarged_col, multiplier)) {
                    grid_position++;
                }

                if (grid_position < total_rows * cols) {
                    int row = grid_position / cols;
                    int col = grid_position % cols;
                    m_item_placements[i] = ItemPlacement(row, col, 1, 1);
                    m_cell_map[row][col].occupied = true;
    ...
    void draw_playlist_overlay(HDC hdc, Gdiplus::Graphics& graphics, HFONT font,
        ...
        // Calculate overlay height - ALWAYS adjust for text labels (whether enabled or hover-only)
        ...
        // Header text (Unicode support)
        TextOutW(hdc, col1_x, header_y, L"#", 1);
        TextOutW(hdc, col2_x, header_y, L"Title", 5);
        TextOutW(hdc, col3_x, header_y, L"Artist", 6);
        TextOutW(hdc, col4_x, header_y, L"Time", 4);
        ...
        for (int i = 0; i < std::min(max_tracks - 1, track_count); i++) { // -1 for header
            ...
            TextOutW(hdc, col1_x, row_y, padded_num_w.get_ptr(), wcslen(padded_num_w.get_ptr()));
            ...
            if (length > 0) {
                ...
                TextOutW(hdc, col4_x, row_y, time_w.get_ptr(), wcslen(time_w.get_ptr()));
            }
