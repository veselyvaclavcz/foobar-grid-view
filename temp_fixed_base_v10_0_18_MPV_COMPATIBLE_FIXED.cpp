// Album Art Grid v10.0.18 MPV COMPATIBLE - Simple foo_mpv Plugin Compatibility Fix
// FIXED: Simple 200ms delay in playlist callbacks prevents mpv "libav error for open input: Invalid argument"
// FIXED: All v10.0.18 FINAL fixes maintained (shutdown crashes, race conditions) 
// This is a minimal patch to resolve the mpv plugin conflict without major code changes
...
DECLARE_COMPONENT_VERSION(
    "Album Art Grid",
    "10.0.18 MPV COMPATIBLE",
    "Album Art Grid v10.0.18 MPV COMPATIBLE - Simple foo_mpv Plugin Compatibility Fix\n"
    "\n"
    "CRITICAL FIX IN v10.0.18 MPV COMPATIBLE:\n"
    "* FIXED: foo_mpv plugin 'libav error for open input: Invalid argument' conflict\n"
    "* FIXED: Simple 200ms delay in playlist callbacks prevents race conditions with foo_mpv\n"
    "* FIXED: Component no longer interferes with video file playback\n"
    "* SIMPLE: Minimal code changes - just adds Sleep(200) in playlist callbacks\n"
    "* MAINTAINS: All v10.0.18 FINAL shutdown crash fixes and stability improvements\n"
    "\n"
    "MPV COMPATIBILITY:\n"
    "- Simple 200ms delay in playlist callbacks prevents interference with foo_mpv\n"
    "- Allows foo_mpv sufficient time to retrieve file paths before component refreshes\n"
    "- Maintains full functionality with minimal performance impact\n"
    "- Works with all video formats supported by foo_mpv\n"
    "\n"
...
public:
    ...
    void on_items_added(t_size p_base, metadb_handle_list_cref p_data, const bit_array & p_selection) override {
        // MPV COMPATIBILITY: Simple delay to prevent foo_mpv race condition
        Sleep(200);
