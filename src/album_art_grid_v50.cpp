// Album Art Grid v10.0.18 - Navigation Enhanced with Unicode Support
// GLOBAL SHUTDOWN PROTECTION: Complete window closing crash prevention
...
DECLARE_COMPONENT_VERSION(
    ...
    "10.0.18",
    "Album Art Grid v10.0.18 - Navigation Enhanced with Unicode Support\n"
    "NEW FEATURES IN v10.0.18:\n"
    "- NAVIGATION ENHANCEMENT: Added Page Up/Page Down keyboard navigation\n"
    "- LETTER JUMP NAVIGATION: Jump to albums by pressing A-Z or 0-9 keys\n" 
...
namespace shutdown_protection {
    ...
    static void initiate_global_shutdown() {
...
public:
    ...
    ~album_grid_instance() {
        ...
        console::print("[Album Art Grid v10.0.12] Closing grid instance");
    ...
    // Unicode Character Display Fix: Helper function for proper UTF-8 to UTF-16 conversion
    void DrawTextUnicode(HDC hdc, const char* utf8_text, int length, LPRECT rect, UINT format) {
        if (!utf8_text || !*utf8_text) return;
        
        // Convert UTF-8 to UTF-16 using MultiByteToWideChar for accurate conversion
        int wide_len = MultiByteToWideChar(CP_UTF8, 0, utf8_text, length, NULL, 0);
        if (wide_len > 0) {
            std::vector<wchar_t> wide_text(wide_len + 1);
            MultiByteToWideChar(CP_UTF8, 0, utf8_text, length, wide_text.data(), wide_len);
            wide_text[wide_len] = 0;
            
            // Use DrawTextW for native Unicode rendering
            DrawTextW(hdc, wide_text.data(), wide_len, rect, format);
        } else {
            // Fallback to ANSI if UTF-8 conversion fails
            DrawTextA(hdc, utf8_text, length, rect, format);
        }
    }
    
    // Overload for TCHAR* (Windows TEXT macro)
    void DrawTextUnicode(HDC hdc, const TCHAR* text, int length, LPRECT rect, UINT format) {
        if (!text || !*text) return;
        
        #ifdef UNICODE
            // Already wide string, use directly
            DrawTextW(hdc, text, length, rect, format);
        #else
            // Convert ANSI to wide
            int wide_len = MultiByteToWideChar(CP_ACP, 0, text, length, NULL, 0);
            if (wide_len > 0) {
                std::vector<wchar_t> wide_text(wide_len + 1);
                MultiByteToWideChar(CP_ACP, 0, text, length, wide_text.data(), wide_len);
                wide_text[wide_len] = 0;
                DrawTextW(hdc, wide_text.data(), wide_len, rect, format);
            } else {
                DrawTextA(hdc, text, length, rect, format);
            }
        #endif
    }
    ...
    LRESULT on_keydown(WPARAM key) {
        if (key == 'P') {
            ...
            // Letter jump navigation A-Z
            jump_to_letter((char)key);
            return 0;
        } else if (key >= '0' && key <= '9') {
            // Number jump navigation 0-9
            jump_to_letter((char)key);
            return 0;
        }
        return 0;
    ...
    void jump_to_letter(char letter) {
        // Jump to first item that starts with given letter
        size_t count = get_item_count();
        if (count == 0) return;
        ...
        for (size_t i = 0; i < count; i++) {
            ...
            // Check first character of display name
            const char* name = item->display_name.c_str();
            if (name && *name) {
                char first_char = toupper(*name);
                if (first_char == upper_letter) {
                    // Found matching item, scroll to it
                    int rows_per_page = max(1, (GetClientRect().bottom - GetClientRect().top - 40) / (m_item_size + PADDING));
                    int target_row = i / m_config.columns;
                    int target_scroll = max(0, target_row - rows_per_page / 2) * (m_item_size + PADDING);
                    
                    SetScrollPos(m_hwnd, SB_VERT, target_scroll, TRUE);
                    InvalidateRect(m_hwnd, NULL, FALSE);
                    return;
                }
            }
                ...
                try {
                    ...
                    if (item->artwork.is_valid()) {
                        ...
                        if (bmp && !shutdown_protection::g_is_shutting_down) {
                            ...
                            console::print("[Album Art Grid v10.0.12] Failed to create thumbnail");
                        }
                    }
                } catch (...) {}
            } catch (...) {}
    ...
    LRESULT on_paint() {
        ...
        if (item_count == 0) {
            ...
            DrawTextUnicode(memdc, message, -1, &rc, DT_CENTER | DT_VCENTER | DT_SINGLELINE);
    ...
    void draw_track_count_badge(HDC hdc, int count, int x, int y, bool dark_mode, HFONT app_font) {
        ...
        DrawTextUnicode(hdc, count_str, -1, &text_rc, DT_CENTER | DT_VCENTER | DT_SINGLELINE);
    ...
    void draw_item(HDC hdc, Gdiplus::Graphics& graphics, HFONT font,
        ...
        if (item->thumbnail->bitmap) {
            ...
            if (item->thumbnail->loading) {
                DrawTextUnicode(hdc, "⏳", -1, &placeholder_rc, DT_CENTER | DT_VCENTER | DT_SINGLELINE);
        ...
        if (m_config.show_text) {
            ...
            DrawTextUnicode(hdc, text_to_show.c_str(), -1, &text_rc, format);
        }
    }
                ...
                if (!m_selected_indices.empty()) {
                    ...
                    if (item && !item->path.is_empty()) {
                        try {
                            ...
                            pfc::stringcvt::string_os_from_utf8 folder_path_w(folder_path.c_str());
                            HINSTANCE result = ShellExecute(m_hwnd, L"explore", folder_path_w, NULL, NULL, SW_SHOWNORMAL);
                            
                            if ((INT_PTR)result <= 32) {
                                // Fallback: try opening parent directory if direct open failed
                                ShellExecute(m_hwnd, L"open", folder_path_w, NULL, NULL, SW_SHOWNORMAL);
                            }
                        } catch (...) {
...
extern "C" {
    void set_global_shutdown_flag() {
        console::print("[Album Art Grid v10.0.12] foobar2000 is shutting down - protecting all instances");
        shutdown_protection::initiate_global_shutdown();
    }
