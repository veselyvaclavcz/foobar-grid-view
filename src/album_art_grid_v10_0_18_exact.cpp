// Album Art Grid v10.0.50 - Full x64 Release
...
DECLARE_COMPONENT_VERSION(
    ...
    "All original v10.0.50 fixes maintained:\n"
...
public:
    ...
    LRESULT on_keydown(WPARAM key) {
        ...
        } else if (key == VK_PRIOR) {  // Page Up navigation
            return on_vscroll(SB_PAGEUP);
        } else if (key == VK_NEXT) {  // Page Down navigation
    ...
    void jump_to_letter(char letter) {
        ...
        for (size_t i = 0; i < item_count; i++) {
            ...
            if (first_char == target) {
                ...
                int rows = (total_items + max(1, m_config.columns) - 1) / max(1, m_config.columns);
                int total_height = rows * (item_height) + PADDING;
