    ...
    void jump_to_letter(char letter) {
        ...
        for (size_t i = 0; i < item_count; i++) {
            ...
            if (first_char == target) {
                ...
                int center_offset = (si.nPage / 2) - (item_height / 2);
                int new_pos = target_y - center_offset;
                
                // Clamp to valid range
                new_pos = max(0, min(new_pos, si.nMax - (int)si.nPage + 1));
                
                if (new_pos != si.nPos) {
                    SetScrollPos(m_hwnd, SB_VERT, new_pos, TRUE);
                    ScrollWindow(m_hwnd, 0, si.nPos - new_pos, NULL, NULL);
                    UpdateWindow(m_hwnd);
                }
                break;
            }
