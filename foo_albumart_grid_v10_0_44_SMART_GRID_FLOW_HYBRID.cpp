// Album Art Grid v10.0.44 - SMART GRID FLOW (stability + async loads)
...
DECLARE_COMPONENT_VERSION(
    "Album Art Grid",
    "10.0.44",
    "Album Art Grid v10.0.44 - SMART GRID FLOW\n"
    "NEW in v10.0.44:\n"
    "- Stability: atomic shutdown flags and thread-safe cache\n"
    "- Performance: async artwork decoding off UI thread\n"
    "- Responsiveness: 64-bit tick counters for timers\n"
    "- Fixes: AlphaBlend link, logging format fixes, safer tooltips\n"
    "- Maintains v10.0.43 SMART GRID FLOW and prior improvements\n"
...
public:
    ...
    static void add_thumbnail(thumbnail_data* thumb, int item_index) {
        if (!thumb || !thumb->bitmap || shutdown_in_progress.load()) return;  // v10.0.9: F9FCh fix
        insync(cache_sync);
    ...
    static bool evict_lru_thumbnail(int current_item_index) {
        ...
        ULONGLONG oldest_time = std::numeric_limits<ULONGLONG>::max();
        int oldest_index = -1;
...
public:
    ...
    void load_visible_artwork() {
        ...
        for (int i = m_first_visible; i <= m_last_visible && i < (int)item_count && load_count < 3; i++) {
            ...
            metadb_handle_ptr track0 = item->tracks.get_count() > 0 ? item->tracks[0] : nullptr;
            auto art_api = album_art_manager_v2::get();

            std::thread([this, hwnd, task_index, gen, enlarged_mode, use_artist_img, track0, art_api]() {
        ...
        if (load_count < 5 && prefetch_start <= prefetch_end) {
            for (int i = prefetch_start; i <= prefetch_end && i < (int)item_count && load_count < 5; i++) {
                ...
                metadb_handle_ptr track0 = item->tracks.get_count() > 0 ? item->tracks[0] : nullptr;
                auto art_api = album_art_manager_v2::get();
                std::thread([this, hwnd, task_index, gen, enlarged_mode, track0, art_api]() {
                    Gdiplus::Bitmap* bmp = nullptr;
                    int size_for_item = 0;
                    try {
                        if (!hwnd || !IsWindow(hwnd)) throw 0;
    ...
    LRESULT on_thumbnail_ready(ThumbnailResult* res) {
        ...
        thumbnail_cache::add_thumbnail(item->thumbnail.get(), res->index);
        InvalidateRect(m_hwnd, NULL, FALSE);
