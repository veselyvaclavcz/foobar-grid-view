// Album Art Grid v10.0.18 MPV RETRY FIX - Fix Infinite Album Art Loading Attempts  
// FIXED: Prevents infinite retry attempts for missing album art that causes foo_mpv libav errors
// FIXED: All v10.0.18 FINAL fixes maintained (shutdown crashes, race conditions)
// This version adds blacklist cache for failed album art loading attempts
...
DECLARE_COMPONENT_VERSION(
    "Album Art Grid",
    "10.0.18 MPV RETRY FIX",
    "Album Art Grid v10.0.18 MPV RETRY FIX - Fix Infinite Album Art Loading Attempts\n"
    "\n"
    "CRITICAL FIX IN v10.0.18 MPV RETRY FIX:\n"
    "* FIXED: Infinite retry attempts for missing album art causing foo_mpv 'libav error'\n"
    "* NEW: Blacklist cache for failed album art attempts - remembers items without covers\n"
    "* NEW: Items without album art are marked and not retried exponentially\n"
    "* IMPROVED: Progressive loading timer stops retrying failed items\n"
    "* MAINTAINS: All v10.0.18 shutdown crash fixes and stability improvements\n"
    "\n"
    "RETRY FIX SOLUTION:\n"
    "- Tracks failed album art loading attempts in blacklist cache\n"
    "- Prevents repeated calls to album_art_manager for items without covers\n"
    "- Stops exponential retry behavior that floods foo_mpv with requests\n"
    "- Items are marked as 'no_art_available' after first failed attempt\n"
    "- Dramatic reduction in console spam and CPU usage\n"
    "\n"
...
struct thumbnail_data {
    ...
    bool should_retry() const {
        // MPV RETRY FIX: Only retry if not blacklisted or after 5 minutes (to allow for library updates)
        if (!no_art_available) return true;
        return (GetTickCount() - blacklist_time) > 300000; // 5 minutes in milliseconds
    }
    ...
    void load_visible_artwork() {
        ...
        for (int i = m_first_visible; i <= m_last_visible && i < (int)item_count && load_count < 3; i++) {
            ...
            // Thread-safe check for loading state
            {
                ...
                // MPV RETRY FIX: Check if this item is blacklisted (no artwork available)
                if (!item->thumbnail->should_retry()) {
                    continue; // Skip this item - it's known to have no artwork
                }
            ...
            try {
                ...
                try {
                    ...
                    // MPV RETRY FIX: If no artwork found, mark as blacklisted to prevent infinite retries
                    if (!item->artwork.is_valid()) {
                        insync(g_thumbnail_sync);
                        if (item && item->thumbnail) {
                            item->thumbnail->set_no_art_available();
                        }
                ...
                try {
                    ...
                    try {
                        ...
                        if (item->artwork.is_valid()) {
                            ...
                            // MPV RETRY FIX: Mark as blacklisted if no artwork found during prefetch
                            insync(g_thumbnail_sync);
                            if (item && item->thumbnail) {
                                item->thumbnail->set_no_art_available();
                            }
...
public:
    ...
    void activate() override {
        ...
        try {
            ...
            console::print("[Album Art Grid] Library viewer activated - add Album Art Grid through View > Layout");
