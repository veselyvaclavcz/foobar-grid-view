// Fixed initquit service for v9.9.9 - better shutdown handling
...
public:
    void on_init() override {
        ...
        if (status == Gdiplus::Ok && m_gdiplusToken != 0) {
            ...
            console::print("Enhanced shutdown protection enabled");
        } else {
            console::print("Album Art Grid v9.9.9: GDI+ initialization failed");
            m_gdiplusToken = 0;
        }
    }
...
// Export the shutdown check function for use in main component
extern "C" __declspec(dllexport) bool albumart_grid_is_shutting_down() {
    return initquit_gdiplus::is_shutting_down();
}