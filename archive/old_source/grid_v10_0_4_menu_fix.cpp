// Album Art Grid v10.0.3 - Path Error Handling Fix
// Fixed metadata extraction errors that prevented album art loading
...
DECLARE_COMPONENT_VERSION(
    ...
    "- One bad path no longer breaks entire library view\n"
    "- Better fallback to folder names when metadata fails\n"
    "From v10.0.2:\n"
...
struct grid_config {
    ...
    enum label_format {
        LABEL_ALBUM_ONLY = 0,
        LABEL_ARTIST_ALBUM = 1,
        LABEL_FOLDER_NAME = 2  // NEW: Added explicit folder name option
    };
    ...
    void load(ui_element_config::ptr cfg) {
        if (cfg.is_valid()) {
            ...
            if (actual_size >= expected_size - sizeof(view_mode) - sizeof(doubleclick_action)) {
                ...
                // v10.0.4: Validate label_style
                if ((int)label_style < 0 || (int)label_style > LABEL_FOLDER_NAME) label_style = LABEL_ARTIST_ALBUM;
            }
        }
    }
    ...
    pfc::string8 get_display_text(grid_config::label_format format) const {
        switch (format) {
            ...
            case grid_config::LABEL_FOLDER_NAME:
                return display_name;  // This is the folder name
...
public:
        ...
        for (t_size i = 0; i < all_items.get_count(); i++) {
            ...
            if (m_config.grouping == grid_config::GROUP_BY_FOLDER) {
                ...
                try {
                    ...
                    console::print("[Album Art Grid v10.0.3] Warning: Failed to get metadata for an item, using folder name");
                }
                ...
                if (!got_from_metadata) {
                    try {
                        ...
                        console::print("[Album Art Grid v10.0.3] Warning: Failed to process path for an item");
                        key = "Unknown";
                        display_name = key;
                    }
                }
                ...
                try {
                    ...
                    if (item->artwork.is_valid()) {
                        ...
                        if (bmp) {
                            ...
                            console::print("[Album Art Grid v10.0.2] Failed to create thumbnail");
                        }
                    }
                } catch (...) {}
            } catch (...) {}
    ...
    LRESULT on_contextmenu(int x, int y) {
        ...
        AppendMenu(label_menu, MF_STRING | (m_config.label_style == grid_config::LABEL_ARTIST_ALBUM ? MF_CHECKED : 0), 38, TEXT("Artist - Album"));
        AppendMenu(label_menu, MF_STRING | (m_config.label_style == grid_config::LABEL_FOLDER_NAME ? MF_CHECKED : 0), 39, TEXT("Folder Name"));
        ...
        switch (cmd) {
            ...
            case 38: m_config.label_style = grid_config::LABEL_ARTIST_ALBUM; config_changed = true; break;
            case 39: m_config.label_style = grid_config::LABEL_FOLDER_NAME; config_changed = true; break;
            case 40: // Select all
                m_selected_indices.clear();
