...
DECLARE_COMPONENT_VERSION(
    "Album Art Grid",
    "8.3.0",
    "Album Art Grid v8.3 with Fluid Dynamic Resizing\n"
    ...
    "\n"
    "Right-click for Resize Mode options"
);
...
struct grid_config {
    ...
    grid_config() : item_size(120), text_lines(2), show_text(true), show_track_count(false),
                    ...
                    resize_mode(ResizeMode::AUTO_FIT), min_item_size(80), max_item_size(200),
                    maintain_aspect_ratio(true), target_columns(0), dynamic_padding(8) {}
...
private:
    ...
    int m_calculated_item_size;
    int m_actual_columns;
    int m_actual_padding;
...
public:
    album_grid_instance(ui_element_config::ptr config, ui_element_instance_callback_ptr callback)
        : m_callback(callback), m_hwnd(NULL), m_parent(NULL), m_scroll_pos(0), 
          ...
          m_calculated_item_size(120), m_actual_columns(1), m_actual_padding(PADDING) {
    ...
    void calculate_fluid_layout() {
        ...
        if (available_width <= 0) {
            ...
            m_actual_columns = 1;
            m_actual_padding = m_config.dynamic_padding;
            return;
        }
        ...
        switch (m_config.resize_mode) {
            ...
            case ResizeMode::AUTO_FIT: {
                ...
                for (int cols = 1; cols <= 20; cols++) {
                    ...
                    int total_padding = m_config.dynamic_padding * (cols + 1);
            ...
            case ResizeMode::AUTO_FILL: {
                ...
                if (target_cols <= 0) {
                    ...
                    target_cols = max(1, available_width / (m_config.item_size + m_config.dynamic_padding));
                }
                
                // Calculate exact item size to fill width
                int total_padding = m_config.dynamic_padding * (target_cols + 1);
                int item_space = available_width - total_padding;
            ...
            case ResizeMode::FLUID: {
                ...
                int best_size = m_config.item_size;
                int best_padding = m_config.dynamic_padding;
                int best_cols = 1;
    ...
    LRESULT on_paint() {
        ...
        // Calculate grid layout with fluid resizing
        ...
        int visible_rows = (rc.bottom + item_total_height + m_actual_padding - 1) / (item_total_height + m_actual_padding) + 1;
        int first_row = m_scroll_pos / (item_total_height + m_actual_padding);
        int y_offset = -(m_scroll_pos % (item_total_height + m_actual_padding));
        ...
        // Draw items with fluid positioning
        // Calculate centered positioning for perfect alignment
        ...
        for (int row = 0; row < visible_rows; row++) {
            for (int col = 0; col < cols; col++) {
                ...
                int y = row * (item_total_height + m_actual_padding) + m_actual_padding + y_offset;
    ...
    void draw_item(HDC hdc, Gdiplus::Graphics& graphics, HFONT font,
        ...
        if (item->thumbnail->bitmap) {
            graphics.DrawImage(item->thumbnail->bitmap, x, y, m_calculated_item_size, m_calculated_item_size);
            item->thumbnail->last_access = GetTickCount();
        } else {
            // Draw placeholder
            ...
            HFONT icon_font = CreateFont(m_calculated_item_size / 4, 0, 0, 0, FW_NORMAL, FALSE, FALSE, FALSE,
                DEFAULT_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS,
        ...
        Rectangle(hdc, x, y, x + m_calculated_item_size, y + m_calculated_item_size);
        ...
        if (m_config.show_text) {
            RECT text_rc = {x, y + m_calculated_item_size + 2, x + m_calculated_item_size, 
                           y + m_calculated_item_size + text_height};
    ...
    LRESULT on_contextmenu(int x, int y) {
        ...
        // Resize mode menu (NEW!)
        HMENU resize_menu = CreatePopupMenu();
        AppendMenu(resize_menu, MF_STRING | (m_config.resize_mode == ResizeMode::FIXED ? MF_CHECKED : 0), 
                   60, TEXT("Fixed Size"));
        AppendMenu(resize_menu, MF_STRING | (m_config.resize_mode == ResizeMode::AUTO_FIT ? MF_CHECKED : 0), 
                   61, TEXT("Auto-Fit (Minimize Gaps)"));
        AppendMenu(resize_menu, MF_STRING | (m_config.resize_mode == ResizeMode::AUTO_FILL ? MF_CHECKED : 0), 
                   62, TEXT("Auto-Fill (Stretch to Fit)"));
        AppendMenu(resize_menu, MF_STRING | (m_config.resize_mode == ResizeMode::FLUID ? MF_CHECKED : 0), 
                   63, TEXT("Fluid (Dynamic Everything)"));
        AppendMenu(menu, MF_POPUP, (UINT_PTR)resize_menu, TEXT("Resize Mode"));
        
        // Size options (only show for Fixed mode)
        HMENU size_menu = nullptr;
        if (m_config.resize_mode == ResizeMode::FIXED) {
            size_menu = CreatePopupMenu();
        ...
        AppendMenu(text_menu, MF_STRING | (m_config.text_lines == 1 ? MF_CHECKED : 0), 70, TEXT("Single Line"));
        AppendMenu(text_menu, MF_STRING | (m_config.text_lines == 2 ? MF_CHECKED : 0), 71, TEXT("Two Lines"));
        AppendMenu(text_menu, MF_STRING | (m_config.text_lines == 3 ? MF_CHECKED : 0), 72, TEXT("Three Lines"));
        ...
        if (size_menu) DestroyMenu(size_menu);
        DestroyMenu(resize_menu);
        ...
        switch (cmd) {
            ...
            case 60: m_config.resize_mode = ResizeMode::FIXED; config_changed = true; break;
            case 61: m_config.resize_mode = ResizeMode::AUTO_FIT; config_changed = true; break;
            case 62: m_config.resize_mode = ResizeMode::AUTO_FILL; config_changed = true; break;
            case 63: m_config.resize_mode = ResizeMode::FLUID; config_changed = true; break;
            
            // Text lines
            case 70: m_config.text_lines = 1; config_changed = true; break;
            case 71: m_config.text_lines = 2; config_changed = true; break;
            case 72: m_config.text_lines = 3; config_changed = true; break;
        }
