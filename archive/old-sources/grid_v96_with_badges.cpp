...
DECLARE_COMPONENT_VERSION(
    "Album Art Grid",
    "9.7.0",
    "Album Art Grid v9.7.0 - Enhanced Badge & Tooltip Edition\n"
    "NEW in v9.7:\n"
    "- Track count badge ALWAYS visible when enabled (not just when labels hidden)\n"
    "- Tooltip shows full text when hovering over truncated labels\n"
    "- Badge uses exact UI font for perfect consistency\n"
    "Features:\n"
...
VALIDATE_COMPONENT_FILENAME("foo_albumart_grid_v97.dll");
...
struct grid_config {
    ...
    grid_config() : columns(5), text_lines(2), show_text(true), show_track_count(true),
                    font_size(11), grouping(GROUP_BY_FOLDER), sorting(SORT_BY_NAME) {}
    ...
    void load(ui_element_config::ptr cfg) {
        if (cfg.is_valid() && cfg->get_data_size() >= sizeof(grid_config)) {
            ...
            // Validate ranges
            if (cfg->get_data_size() == sizeof(int) * 7) {
                // Old config - convert to columns
                columns = 5;
            } else {
...
public:
    album_grid_instance(ui_element_config::ptr config, ui_element_instance_callback_ptr callback)
        : m_callback(callback), m_hwnd(NULL), m_parent(NULL), m_tooltip(NULL), m_scroll_pos(0), 
    ...
    LRESULT on_mousemove(int x, int y) {
        ...
        if (index != m_hover_index) {
            ...
            // Update tooltip - show when labels are hidden OR when hovering over text
            if (m_tooltip && index >= 0 && index < (int)m_items.size()) {
                auto& item = m_items[index];
                ...
                if (!m_config.show_text) {
                    // Always show tooltip when labels are hidden
                    show_tooltip = true;
                } else {
                    // When labels are shown, check if mouse is over text area
                    RECT rc;
                    GetClientRect(m_hwnd, &rc);
                    calculate_layout();
                    int text_height = calculate_text_height();
                    ...
                    // Check if cursor is in text area
                    RECT text_rc = {item_x - 2, item_y + m_item_size + 4, 
                                   item_x + m_item_size + 2, item_y + m_item_size + text_height - 2};
                    
                    POINT cursor_pt = {x, y};
                    if (PtInRect(&text_rc, cursor_pt)) {
                        show_tooltip = true;
                    }
                }
                ...
                if (show_tooltip) {
                    ...
                    POINT pt = {x, y + 20};  // Position below cursor
            ...
            } else if (m_tooltip && m_tooltip_index != -1) {
