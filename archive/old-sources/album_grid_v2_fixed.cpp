...
// Fixed version with proper API compliance
...
extern "C" {
    // CRITICAL FIX: Return NULL for v2 interface - this prevents the crash
    // foobar2000 v2 expects components to return NULL if they don't implement the requested interface
    __declspec(dllexport) void* foobar2000_get_interface(const char* name, unsigned version) {
        // For now, return NULL to indicate we don't provide any services
        // This is valid for a minimal component that doesn't add functionality
        return NULL;
    }
    
    // Version info - required for component to be recognized
    __declspec(dllexport) const char* foobar2000_component_version() {
        return "2.0.1";
    }
    
    // API version we're built against
    __declspec(dllexport) int foobar2000_get_interface_version() {
        return FOOBAR2000_TARGET_VERSION;
    }
    
    // Create window for testing (optional export for manual testing)
