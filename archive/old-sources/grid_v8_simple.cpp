...
DECLARE_COMPONENT_VERSION(
    "Album Art Grid",
    "8.5.0",
    "Album Art Grid v8.5 - Auto-Fill with Column Control\n"
    "Automatically fills container width\n"
    "Ctrl+Mouse Wheel adjusts columns (2-10)\n"
    ...
    void load(ui_element_config::ptr cfg) {
        if (cfg.is_valid() && cfg->get_data_size() >= sizeof(grid_config)) {
            ...
            if (cfg->get_data_size() == sizeof(int) * 7) {
                // Old config with item_size - convert to columns
                int old_item_size = *(int*)cfg->get_data();
                columns = 5;  // Default columns
            } else {
