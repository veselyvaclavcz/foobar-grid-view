...
private:
    ...
    void draw_track_count_badge(HDC hdc, int count, int x, int y, bool dark_mode) {
        ...
        // Draw badge background - simple rounded rect with GDI
        HBRUSH hbrBadge = CreateSolidBrush(dark_mode ? RGB(60, 60, 60) : RGB(255, 60, 60));
        HPEN hpenBadge = CreatePen(PS_SOLID, 1, dark_mode ? RGB(80, 80, 80) : RGB(200, 50, 50));
        HBRUSH hOldBrush = (HBRUSH)SelectObject(hdc, hbrBadge);
        HPEN hOldPen = (HPEN)SelectObject(hdc, hpenBadge);
        
        RoundRect(hdc, badge_x, badge_y, badge_x + badge_width, badge_y + badge_height, 10, 10);
        ...
        RECT text_rect = { badge_x, badge_y, badge_x + badge_width, badge_y + badge_height };
        DrawTextA(hdc, count_str, -1, &text_rect, DT_CENTER | DT_VCENTER | DT_SINGLELINE);
    }
...
DECLARE_COMPONENT_VERSION(
    ...
    "Enhanced with track count badges!\n\n"
    "NEW in v9.6:\n"
