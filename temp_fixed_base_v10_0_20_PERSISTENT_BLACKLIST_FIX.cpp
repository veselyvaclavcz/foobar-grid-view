// Album Art Grid v10.0.19 MPV FINAL - Definitive Fix for Album Art Loading  
// FIXED: Album art is now requested ONLY ONCE per item - no retries, no repeated requests
// FIXED: Timer properly stops even when items have no artwork
// FIXED: Permanent blacklist - items without art are never retried
...
DECLARE_COMPONENT_VERSION(
    ...
    "* FIXED: Timer properly stops even when some items have no artwork\n"
    "* FIXED: Permanent blacklist - items without art are NEVER retried\n"
    "* FIXED: No more 'libav error' spam from foo_mpv\n"
    "* MAINTAINS: All v10.0.18 shutdown crash fixes and stability improvements\n"
    "\n"
    "DEFINITIVE SOLUTION:\n"
    "- Each item gets exactly ONE attempt to load album art\n"
    "- If no art is found, the item is permanently blacklisted\n"
    "- Progressive timer checks if there are items to load, not if all have images\n"
    "- Timer stops when no more items need loading (not when all have images)\n"
    "- Result: Zero repeated requests, zero console spam\n"
    "\n"
    "MAINTAINED FROM v10.0.18 FINAL:\n"
    ...
    "- FIXED: Crash at offset 0x1208Ch - object validation before pointer access\n"
    "- FIXED: Corrupted callback pointers causing access violations\n"
    "- FIXED: Version string now correctly shows v10.0.17\n"
...
// Global video playback detection for MPV compatibility
static std::atomic<bool> g_is_playing_video{false};

// Function to check if file is video
inline bool is_video_file(const char* path) {
    ...
    const char* video_exts[] = {
        "mp4", "mkv", "avi", "webm", "mov", "flv", "wmv", 
        "mpg", "mpeg", "m4v", "3gp", "ogv", "vob", "ts", 
        "m2ts", "mts", "divx", "xvid", "rm", "rmvb", "asf"
    };
    ...
    for (const char* video_ext : video_exts) {
        if (strcmp(ext_lower, video_ext) == 0) {
            return true;
        }
    }
...
// Play callback to detect video playback
class play_callback_mpv_detector : public play_callback_static {
public:
    ...
    void on_playback_new_track(metadb_handle_ptr p_track) override {
        if (p_track.is_valid()) {
            ...
            g_is_playing_video.store(is_video);
            
            if (is_video) {
                console::print("Album Art Grid: Video playback detected, disabling playlist callbacks");
            }
        }
    }
    
    void on_playback_stop(play_control::t_stop_reason p_reason) override {
        if (g_is_playing_video.load()) {
            console::print("Album Art Grid: Video playback stopped, re-enabling playlist callbacks");
            g_is_playing_video.store(false);
        }
    }
...
static play_callback_static_factory_t<play_callback_mpv_detector> g_play_callback_mpv_detector;
...
// v10.0.20 PERSISTENT BLACKLIST FIX: Enhanced blacklist that survives component refreshes
class persistent_blacklist {
private:
    // GLOBAL STATIC STORAGE - persists across all component operations and refreshes
    static std::unordered_set<std::string> g_blacklisted_items;
    static critical_section g_blacklist_sync;

public:
    // Enhanced key generation for better uniqueness (v10.0.20 improvement)
    static std::string make_blacklist_key(const char* artist, const char* album, const char* path = nullptr) {
        std::string key;
        if (artist && *artist) {
            key += "artist:";
            key += artist;
    ...
    // Thread-safe blacklist check (survives refreshes)
    ...
    // Thread-safe blacklist addition (persists across refreshes)
    static void add_to_blacklist(const std::string& key) {
        insync(g_blacklist_sync);
        g_blacklisted_items.insert(key);
        console::printf("v10.0.20 Persistent Blacklist: Added '%s' (Total: %zu items)", 
    ...
    static void clear_blacklist() {
        ...
        console::printf("v10.0.20 Persistent Blacklist: Cleared %zu items", count);
    }
    ...
    // Check if key is blacklisted using metadata directly
    static bool is_item_blacklisted(metadb_handle_ptr item) {
        if (!item.is_valid()) return false;
        
        file_info_impl info;
        if (!item->get_info(info)) return false;
        
        const char* artist = info.meta_get("ARTIST", 0);
        const char* album = info.meta_get("ALBUM", 0);
        const char* path = item->get_path();
        
        std::string key = make_blacklist_key(artist, album, path);
        return is_blacklisted(key);
    }

    // Add item to blacklist using metadata directly  
    static void blacklist_item(metadb_handle_ptr item) {
        if (!item.is_valid()) return;
        
        file_info_impl info;
        if (!item->get_info(info)) return;
...
// GLOBAL STATIC STORAGE DEFINITIONS - PERSISTENT ACROSS ALL REFRESHES
...
// Thumbnail cache entry with memory tracking and retry blacklist
struct thumbnail_data {
    ...
    int cached_size;
    size_t memory_size;
    bool no_art_available;  // MPV RETRY FIX: Blacklist flag to prevent infinite retry attempts
    DWORD blacklist_time;   // MPV RETRY FIX: When item was blacklisted
    
    thumbnail_data() : bitmap(nullptr), last_access(GetTickCount()), loading(false), 
                      cached_size(0), memory_size(0), no_art_available(false), blacklist_time(0) {}
    ...
    void set_bitmap(Gdiplus::Bitmap* bmp, int size) {
        ...
        // MPV RETRY FIX: Clear blacklist flag when we successfully get artwork
        no_art_available = false;
        blacklist_time = 0;
    }
    
    void set_no_art_available() {
        // MPV RETRY FIX: Mark as having no artwork to prevent infinite retries
        no_art_available = true;
        blacklist_time = GetTickCount();
        clear(); // Clear any existing bitmap
    }
    
    bool should_retry() const {
        // MPV FINAL FIX: NEVER retry once blacklisted - permanent blacklist
        return !no_art_available;
    }
...
public:
    ...
    LRESULT on_timer(WPARAM timer_id) {
        if (timer_id == TIMER_LOAD) {
            ...
            // MPV FINAL FIX: Check if any items need loading (not if all have bitmaps!)
            ...
            for (int i = m_first_visible; i <= m_last_visible && i < (int)m_items.size(); i++) {
                if (!m_items[i]->thumbnail->bitmap && !m_items[i]->thumbnail->loading && 
                    m_items[i]->thumbnail->should_retry()) {
                    needs_loading = true;
                    break;
                }
            }
    ...
    LRESULT on_keydown(WPARAM key) {
        if (key == 'P') {
            ...
            if (key == 'A') {
                ...
                // v10.0.20: Clear persistent blacklist on Ctrl+B
                persistent_blacklist::clear_blacklist();
                console::print("v10.0.20: Persistent blacklist cleared via Ctrl+B");
                refresh_items(); // Refresh to potentially load previously blacklisted items
                return 0;
            }
    ...
    void load_visible_artwork() {
        ...
        for (int i = m_first_visible; i <= m_last_visible && i < (int)item_count && load_count < 3; i++) {
            ...
            // Thread-safe check for loading state
            {
                ...
                // v10.0.20 PERSISTENT BLACKLIST FIX: Check both local and global blacklists
                if (!item->thumbnail->should_retry()) {
                    continue; // Skip - local blacklist (legacy)
                }
                
                // v10.0.20: Check global persistent blacklist (survives refreshes)
                if (persistent_blacklist::is_item_blacklisted(item->tracks[0])) {
                    continue; // Skip - global persistent blacklist
                }
            ...
            try {
                ...
                try {
                    ...
                    // v10.0.20 PERSISTENT BLACKLIST FIX: Enhanced blacklisting for missing artwork
                    if (!item->artwork.is_valid()) {
                        insync(g_thumbnail_sync);
                        if (item && item->thumbnail) {
                            item->thumbnail->set_no_art_available(); // Local blacklist (legacy)
                        }
                        
                        // v10.0.20: Add to global persistent blacklist (survives refreshes)
                        if (item->tracks.get_count() > 0) {
                            persistent_blacklist::blacklist_item(item->tracks[0]);
                        }
                ...
                try {
                    ...
                    try {
                        ...
                        if (item->artwork.is_valid()) {
                            ...
                            // v10.0.20 PERSISTENT BLACKLIST FIX: Enhanced blacklisting during prefetch
                            insync(g_thumbnail_sync);
                            if (item && item->thumbnail) {
                                item->thumbnail->set_no_art_available(); // Local blacklist (legacy)
                            }
    ...
    void on_items_added(t_size p_base, metadb_handle_list_cref p_data, const bit_array & p_selection) override {
        // MPV FINAL FIX: Skip callbacks completely during video playback
        if (g_is_playing_video.load()) {
            return; // Don't process any playlist callbacks during video playback
        }
...
// Library viewer service - makes the component appear in "Installed media library viewers" list
class album_grid_library_viewer : public library_viewer {
public:
    GUID get_preferences_page() override {
        return pfc::guid_null; // No preferences page for now
    }
    
    bool have_activate() override {
        return true; // We support activation
    }
    
    void activate() override {
        // CRITICAL: Check shutdown protection before accessing any foobar2000 services
        if (shutdown_protection::g_is_shutting_down.load()) {
            console::print("[Album Art Grid] Library viewer activation blocked - foobar2000 is shutting down");
            return;
        }
        ...
        try {
            ...
            // Show message to user about how to add the grid
            popup_message::g_show("To use Album Art Grid:\n\n1. Go to View > Layout > Enable layout editing mode\n2. Right-click in layout area\n3. Add UI Element > Album Art Grid\n\nOr check if Album Art Grid is available in View menu or Library menu.", "Album Art Grid", popup_message::icon_information);
        } catch (...) {
            console::print("[Album Art Grid] Library viewer activation failed - exception caught during shutdown");
        }
    }
    
    GUID get_guid() override {
        // Use the same GUID as our UI element but with slight modification for library viewer
        static const GUID guid = { 0x6b7c5f92, 0xac4e, 0x5d96, { 0xb9, 0x03, 0x8d, 0x4e, 0xac, 0x6f, 0x3b, 0x21 } };
        return guid;
    }
...
static library_viewer_factory_t<album_grid_library_viewer> g_album_grid_library_viewer_factory;
